...
export default class GameScene extends Phaser.Scene {
    ...
    updateScoreboard(scores) {
        ...
        scores.forEach((entry, index) => {
            ...
            let nameVal = entry.playerId.substring(0, 6);
            if (isMe) {
                nameVal = PlayerData.alias ? PlayerData.alias : 'YOU';
            } else {
                const remote = this.otherPlayerMap[entry.playerId];
                if (remote && remote.alias) {
                    nameVal = remote.alias;
                } else if (entry.alias) {
                    nameVal = entry.alias;
                }
            }
        ...
        container.innerHTML = `
            ...
            <div id="ui-player-panel" class="pixel-panel">
                ...
                <div class="player-stats">
                    <h3 class="player-name">${PlayerData.alias ? PlayerData.alias.toUpperCase() : 'PLAYER'}</h3>
                    <div class="hp-bar-bg">
